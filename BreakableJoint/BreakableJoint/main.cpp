...
int main(void)
{
	...
	// create sphere which has initial velocity
	PxRigidDynamic* actor4 = createDynamic(PxTransform((PxVec3(-5.0f, 0.5f, 0.0f))), PxSphereGeometry(0.5f), *material);
	actor4->setLinearVelocity(PxVec3(10.0f, 0.0f, 0.0f));
	...
	PxFixedJoint *joint0 = PxFixedJointCreate(*gPhysics, actor0, PxTransform(joint0Pos - actor0->getGlobalPose().p), actor1, PxTransform(joint0Pos - actor1->getGlobalPose().p));
	PxFixedJoint *joint1 = PxFixedJointCreate(*gPhysics, actor1, PxTransform(joint1Pos - actor1->getGlobalPose().p), actor2, PxTransform(joint1Pos - actor2->getGlobalPose().p));
	PxFixedJoint *joint2 = PxFixedJointCreate(*gPhysics, actor2, PxTransform(joint2Pos - actor2->getGlobalPose().p), actor3, PxTransform(joint2Pos - actor3->getGlobalPose().p));
	PxFixedJoint *joint3 = PxFixedJointCreate(*gPhysics, actor3, PxTransform(joint3Pos - actor3->getGlobalPose().p), actor0, PxTransform(joint3Pos - actor0->getGlobalPose().p));

	// break force/torque setting
	PxReal breakForce = 100;
	PxReal breakTorque = 100;
	joint0->setBreakForce(breakForce, breakTorque);
	joint1->setBreakForce(breakForce, breakTorque);
	joint2->setBreakForce(breakForce, breakTorque);
	joint3->setBreakForce(breakForce, breakTorque);
